...
const migrationFiles = [
  ...
  '0002_add_registration_fields.sql',
  '0003_add_business_address_2.sql',
  '0004_split_contact_name.sql',
  '0005_add_webhook_logs_table.sql'
];
...
async function executeSQLFile(filePath, fileName) {
  ...
  try {
    ...
    // Some migrations might fail if columns already exist - check if it's a benign error
    if (error.message.includes('already exists')) {
      console.log(`⚠️  Warning: ${fileName} - Some objects already exist (skipping)`);
      return true;
    }
  ...
  try {
    ...
    for (const fileName of migrationFiles) {
      ...
      if (fs.existsSync(filePath)) {
        ...
        console.log(`⚠️  Warning: Migration file not found: ${fileName}`);
      }
    }
    ...
    console.log('   3. Create an admin user in admin_users table');
